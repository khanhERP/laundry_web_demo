...
export default function SalesOrders() {
  ...
  useEffect(() => {
    ...
    const handleEInvoiceModalClosed = async (event: CustomEvent) => {
      ...
      queryClient.removeQueries({ queryKey: ["https://laundry-be-demo.onrender.com/api/orders"] });
      queryClient.removeQueries({ queryKey: ["https://laundry-be-demo.onrender.com/api/order-items"] });
      queryClient.removeQueries({ queryKey: ["https://laundry-be-demo.onrender.com/api/invoices"] });

      // Force immediate refetch with fresh data
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["https://laundry-be-demo.onrender.com/api/orders"] }),
        queryClient.invalidateQueries({ queryKey: ["https://laundry-be-demo.onrender.com/api/order-items"] }),
        queryClient.invalidateQueries({ queryKey: ["https://laundry-be-demo.onrender.com/api/invoices"] }),
        queryClient.invalidateQueries({ queryKey: ["https://laundry-be-demo.onrender.com/api/tables"] }),
        queryClient.refetchQueries({ queryKey: ["https://laundry-be-demo.onrender.com/api/orders"] }),
        queryClient.refetchQueries({ queryKey: ["https://laundry-be-demo.onrender.com/api/invoices"] }),
      ]);
  ...
  useEffect(() => {
    const handleOrderUpdate = async () => {
      ...
      await queryClient.refetchQueries({
        queryKey: ["https://laundry-be-demo.onrender.com/api/orders/list"],
        exact: false,
      });
    };
  ...
  const { data: customers = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await apiRequest("GET", "https://laundry-be-demo.onrender.com/api/customers");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const {
    ...
    queryKey: [
      "https://laundry-be-demo.onrender.com/api/orders/list",
      startDate,
      endDate,
      dateFilterMode,
      customerSearch,
    ...
    queryFn: async () => {
      try {
        ...
        const url = `https://laundry-be-demo.onrender.com/api/orders/list?${params.toString()}`;
  ...
  const { data: products = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await apiRequest("GET", "https://laundry-be-demo.onrender.com/api/products");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const { data: tables = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await apiRequest("GET", "https://laundry-be-demo.onrender.com/api/tables");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const {
    ...
    queryKey: ["https://laundry-be-demo.onrender.com/api/order-items", selectedInvoice?.id],
    queryFn: async () => {
      ...
      try {
        const response = await apiRequest(
          "GET",
          `https://laundry-be-demo.onrender.com/api/order-items/${selectedInvoice.id}`,
        );
  ...
  const updateOrderMutation = useMutation({
    mutationFn: async (updatedOrder: Order) => {
      ...
      const itemsResponse = await apiRequest(
        "GET",
        `https://laundry-be-demo.onrender.com/api/order-items/${updatedOrder.id}`,
      );
      ...
      console.log("👤 Thông tin khách hàng trong payload:");
      console.log("  - Tên khách hàng:", updatePayload.customerName);
      console.log("  - Số điện thoại:", updatePayload.customerPhone);
      console.log("  - Địa chỉ:", updatePayload.customerAddress);
      console.log("  - Mã số thuế:", updatePayload.customerTaxCode);
      console.log("  - Email:", updatePayload.customerEmail);
      console.log("📄 Thông tin hóa đơn trong payload:");
      console.log("  - Ký hiệu hóa đơn:", updatePayload.symbol);
      console.log("  - Số hóa đơn:", updatePayload.invoiceNumber);
      console.log("  - Số mẫu hóa đơn:", updatePayload.templateNumber);
      console.log("💰 Thông tin tài chính:");
      console.log("  - Tổng tiền hàng:", updatePayload.subtotal);
      ...
      const response = await apiRequest(
        "PUT",
        `https://laundry-be-demo.onrender.com/api/orders/${updatedOrder.id}`,
        updatePayload,
      );
      ...
      if (response.ok) {
        ...
        console.log("  - symbol:", responseData.symbol);
        console.log("  - invoiceNumber:", responseData.invoiceNumber);
      } else {
        const errorText = await response.text();
    ...
    onSuccess: async (data, updatedOrder) => {
      ...
      await queryClient.refetchQueries({
        queryKey: ["https://laundry-be-demo.onrender.com/api/order-items", updatedOrder.id],
      });
    ...
    mutationFn: async (orderIds: string[]) => {
      ...
      for (const orderId of orderIds) {
        try {
          ...
          const response = await apiRequest(
            "PUT",
            `https://laundry-be-demo.onrender.com/api/orders/${orderId}/status`,
            {
              status: "cancelled",
            },
          );
  ...
  const publishRequestMutation = useMutation({
    mutationFn: async (invoiceData: any) => {
      const response = await apiRequest(
        "POST",
        "https://laundry-be-demo.onrender.com/api/einvoice/publish",
        invoiceData,
      );
      return response.json();
    },
      ...
      if (result.success && selectedInvoice) {
        try {
          ...
          const updateResponse = await apiRequest(
            "PUT",
            `https://laundry-be-demo.onrender.com/api/orders/${selectedInvoice.id}`,
            updateData,
          );
          console.log(
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      // Capture original order data for change history
      const originalOrder = selectedInvoice;
      const originalItems = orderItems;

      // Track if there are any actual changes
      ...
      for (const item of itemsToCreate) {
        ...
        const response = await apiRequest(
          "POST",
          `https://laundry-be-demo.onrender.com/api/order-items/${editableInvoice.id}`,
          payload,
        );
      ...
      console.log("📝 Preparing order update with customer info:", {
        ...
        templateNumber: editableInvoice.templateNumber,
      });
      ...
      const orderUpdateResponse = await apiRequest(
        "PUT",
        `https://laundry-be-demo.onrender.com/api/orders/${editableInvoice.id}`,
        orderUpdatePayload,
      );
      ...
      // Step 7: Build detailed change description
      ...
      if (hasChanges) {
        ...
        try {
          ...
          await apiRequest(
            "POST",
            "https://laundry-be-demo.onrender.com/api/order-change-history",
        ...
        console.log("ℹ️ No changes detected - skipping change history logging");
      }

      // Step 9: Force immediate refresh after ALL operations complete
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["https://laundry-be-demo.onrender.com/api/orders"] }),
        queryClient.refetchQueries({ queryKey: ["https://laundry-be-demo.onrender.com/api/orders/list"] }),
        queryClient.refetchQueries({
          queryKey: ["https://laundry-be-demo.onrender.com/api/order-items", editableInvoice.id],
        }),
        queryClient.refetchQueries({
          queryKey: [
            "https://laundry-be-demo.onrender.com/api/orders/date-range",
            startDate,
            endDate,
            currentPage,
            itemsPerPage,
    ...
    if (orderDiscount >= 0 && visibleItems.length > 0) {
      ...
      if (totalBeforeDiscount > 0) {
        ...
        visibleItems.forEach((item: any, index: number) => {
          ...
          // ✅ ĐƠN GIẢN: Thành tiền = Số lượng × Đơn giá
          const priceBeforeTax = Math.round(unitPrice * quantity);
          ...
          if (orderDiscount > 0) {
            ...
            if (isLastItem) {
              ...
              itemDiscountAmount = Math.floor(
                (orderDiscount * priceBeforeTax) / totalBeforeDiscount,
              );
            }
          }
          ...
          // Calculate tax and total based on settings
          let itemTax = 0;
          let calculatedTotal = 0;

          if (priceIncludeTax) {
            // Nếu giá đã bao gồm thuế: Tổng cộng = Thành tiền - Giảm giá
            calculatedTotal = priceBeforeTax - itemDiscountAmount;
            itemTax = 0; // Không tính thuế riêng vì đã bao gồm trong giá
          } else {
  ...
  // Helper function for handling key presses in order item inputs
  const handleOrderItemKeyDown = (
    ...
    const currentItemId = visibleItems[index]?.id;
    ...
    if (e.key === "Enter") {
      ...
      if (
        ...
        // Moved from last field to first of current row (this shouldn't happen with modulo logic)
      } else {
        // Moved to next field in the same row
        nextField = fieldsOrder[nextFieldIndex];
      }

      // If moving to the next row, check if it exists
      if (nextIndex >= visibleItems.length) {
        // If it's the last item and user presses Enter, potentially add new item or focus on save button
        if (field === "unitPrice") {
          // If user is on unitPrice of the last item and presses Enter, focus on Add New Item button
          if (isEditing) {
            const addButton = document.querySelector(
              'button[title="Thêm dòng mới"]',
            ) as HTMLButtonElement;
            addButton?.focus();
          } else {
            // If not editing, focus on save button
            const saveButton = document.getElementById("save-order-button");
            saveButton?.focus();
          }
          return;
        } else {
          // Otherwise, default to next field if available
          const nextInput = document.querySelector(
            `[data-field="orderitem-${nextField}-${nextIndex}"]`,
          ) as HTMLInputElement;
          nextInput?.focus();
          return;
        }
      ...
      const prevInput = document.querySelector(
        `[data-field="orderitem-${field}-${prevIndex}"]`,
      ) as HTMLInputElement;
      prevInput?.focus();
      ...
      const nextInput = document.querySelector(
        `[data-field="orderitem-${field}-${nextIndex}"]`,
      ) as HTMLInputElement;
      nextInput?.focus();
    }
  };
  ...
  const displayTotals = useMemo(() => {
    ...
    if (isEditing && Object.keys(editedOrderItems).length > 0) {
      ...
      let calculatedSubtotal = 0;
      let calculatedTax = 0;
      let calculatedDiscount = 0; // Calculate from items
      ...
      visibleItems.forEach((item: any) => {
        ...
        // Get discount from item (not from order)
        const itemDiscount = parseFloat(
          editedItem.discount !== undefined
            ? editedItem.discount
            : item.discount || "0",
      ...
      // Calculate discount from order items when not editing
      let itemsDiscount = 0;
      if (orderItems && orderItems.length > 0) {
        itemsDiscount = orderItems.reduce((sum, item) => {
          return sum + parseFloat(item.discount || "0");
        }, 0);
      }
      ...
      return {
        ...
        discount: Math.round(itemsDiscount),
  ...
  const handlePayment = async (order: Invoice) => {
    ...
    try {
      const updateResponse = await apiRequest(
        "PUT",
        `https://laundry-be-demo.onrender.com/api/orders/${order.id}`,
        {
          paymentStatus: "paid",
          status: "paid",
      ...
      if (updateResponse.ok) {
        ...
        if (storeSettings?.businessType === "laundry") {
          ...
          const itemsResponse = await apiRequest(
            "GET",
            `https://laundry-be-demo.onrender.com/api/order-items/${order.id}`,
          );
  ...
  const handlePrintReceipt = async (order: any) => {
    try {
      ...
      const response = await apiRequest("GET", `https://laundry-be-demo.onrender.com/api/order-items/${order.id}`);
                        ...
                        {filteredInvoices.length === 0 ? (
                          ...
                          filteredInvoices
                            ...
                            .map((item) => {
                              ...
                              const customerName = item.customerName || "";
                              const customerPhone = item.customerPhone || ""; // Get customer phone
                              // Discount is already calculated from order_items in API response
                              const discount = parseFloat(item.discount || "0");
                              const tax = parseFloat(item.tax || "0");
                              const subtotal = parseFloat(item.subtotal || "0");
                              const total = parseFloat(item.total || "0");
                                                                    ...
                                                                    <td className="text-right py-2 px-3 border-r text-base w-[100px]">
                                                                      {(() => {
                                                                        ...
                                                                        if (
                                                                          ...
                                                                          // priceIncludeTax: false → Tổng cộng = Thành tiền - Giảm giá + Thuế
                                                                          totalAmount =
                                                                            thanhTien -
                                                                            itemDiscountAmount +
                                                                            itemTax;
